...
public class SpecialBuses extends Activity {

	@Override
	protected void onCreate(Bundle savedInstanceState) {
		super.onCreate(savedInstanceState);
		setContentView(R.layout.activity_special_buses);
		// Show the Up button in the action bar.
		Database db = new Database(this);
        Cursor busFrom = db.checkHoliday();
        List<Button> bus = new ArrayList<Button>(); 
        if(busFrom.getCount() > 0)
        {
       	 busFrom.moveToFirst();
       	 LinearLayout ll = (LinearLayout)findViewById(R.id.layout_specBus);
