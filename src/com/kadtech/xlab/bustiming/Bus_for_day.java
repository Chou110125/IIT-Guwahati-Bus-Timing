...
@SuppressLint("NewApi")
public class Bus_for_day extends Activity {

	@Override
	protected void onCreate(Bundle savedInstanceState) {
		super.onCreate(savedInstanceState);
		setContentView(R.layout.activity_bus_for_day);
		// Show the Up button in the action bar.
		Today today = new Today();
		Database db = new Database(this);
        Cursor busFrom = db.getFrom("00:00", today,50);
        List<Button> bus = new ArrayList<Button>(); 
        if(busFrom.getCount() > 0)
        {
       	 busFrom.moveToFirst();
       	 LinearLayout ll = (LinearLayout)findViewById(R.id.layout_bus_for_day);
